...
mysql -u $USERNAME -p$PASSWORD -h $MYSQL_HOST $DATABASE -e "LOAD DATA LOCAL INFILE '$TMP_DIAGNOSIS_FILE' INTO TABLE prediction_diagnosis FIELDS TERMINATED BY '|';";
...
cat $QUESTION_FILE | sed '1d' | awk 'BEGIN { FS=","; FPAT = "([^,]*)|(\"[^\"]+\")" } {  print ""NR"|"$1"|"$2"|"$3"" }' > "$TMP_QUESTION_FILE"
mysql -u $USERNAME -p$PASSWORD -h $MYSQL_HOST $DATABASE -e "LOAD DATA LOCAL INFILE '$TMP_QUESTION_FILE' INTO TABLE prediction_question FIELDS TERMINATED BY '|' OPTIONALLY ENCLOSED BY '\"';"
...
mysql -u $USERNAME -p$PASSWORD -h $MYSQL_HOST $DATABASE -e "LOAD DATA LOCAL INFILE '$TMP_RESPONSE_FILE' INTO TABLE prediction_response FIELDS TERMINATED BY '|' OPTIONALLY ENCLOSED BY '\"' (id,answer,prediction_id,@var1,priority,question_id) SET is_default = (@var1 = 'True');"
...
mysql -u $USERNAME -p$PASSWORD -h $MYSQL_HOST $DATABASE -e "LOAD DATA LOCAL INFILE '$TMP_LINK_FILE' INTO TABLE prediction_priority FIELDS TERMINATED BY '|' OPTIONALLY ENCLOSED BY '\"';"

while read id type atgard
do
    echo "$id|$atgard|$type" >> "$TMP_RECOMMENDATION_FILE"
done < <(cat $ATGARD_FILE | sed '1,2d' | awk 'BEGIN { FS=","; FPAT = "([^,]+)|(\"[^\"]+\")" } {  print ""$1"|"$2"|"$3"" }')
...
done < <(cat $ATGARD_LINK_FILE | sed '1,2d' | awk 'BEGIN { FS=","; FPAT = "([^,]+)|(\"[^\"]+\")" } {  print ""NR"|"$1"|"$2"|"$3"" }')

mysql -u $USERNAME -p$PASSWORD -h $MYSQL_HOST $DATABASE -e "LOAD DATA LOCAL INFILE '$TMP_MEASURE_FILE' INTO TABLE measure FIELDS TERMINATED BY '|' OPTIONALLY ENCLOSED BY '\"';"
mysql -u $USERNAME -p$PASSWORD -h $MYSQL_HOST $DATABASE -e "LOAD DATA LOCAL INFILE '$TMP_RECOMMENDATION_FILE' INTO TABLE recommendation FIELDS TERMINATED BY '|' OPTIONALLY ENCLOSED BY '\"';"
mysql -u $USERNAME -p$PASSWORD -h $MYSQL_HOST $DATABASE -e "LOAD DATA LOCAL INFILE '$TMP_MEASURE_PRIORITY_FILE' INTO TABLE measure_priority FIELDS TERMINATED BY '|' OPTIONALLY ENCLOSED BY '\"';"
