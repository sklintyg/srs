...
DIAGNOSIS_FILE=$4
TMP_DIAGNOSIS_FILE=/tmp/prediction_diagnosis.txt

QUESTION_FILE=$5
TMP_QUESTION_FILE=/tmp/prediction_question.txt

RESPONSE_FILE=$6
TMP_RESPONSE_FILE=/tmp/prediction_response.txt

LINK_FILE=$7
TMP_LINK_FILE=/tmp/prediction_priority.txt

ATGARD_FILE=$8
TMP_MEASURE_FILE=/tmp/measure.txt
TMP_RECOMMENDATION_FILE=/tmp/recommendation.txt
TMP_MEASURE_PRIORITY_FILE=/tmp/measure_priority.txt
...
cat $DIAGNOSIS_FILE | sed '1d' | awk 'BEGIN { FPAT = "([^,]+)|(\"[^\"]+\")" } {  print ""NR"|"$1"|"$2""}' > "$TMP_DIAGNOSIS_FILE"
...
cat $QUESTION_FILE | sed '1d' | awk 'BEGIN { FPAT = "([^,]+)|(\"[^\"]+\")" } {  print ""NR"|"$1"|"$2"|"$3"" }' > "$TMP_QUESTION_FILE"
...
done < <(cat $RESPONSE_FILE | sed '1d' | awk 'BEGIN { FPAT = "([^,]+)|(\"[^\"]+\")" } {  print ""NR"|"$1"|"$2"|"$3"|"$4"|"$5"|"$6"" }')
...
rm -f $TMP_MEASURE_FILE
rm -f $TMP_MEASURE_PRIORITY_FILE
rm -f $TMP_RECOMMENDATION_FILE
