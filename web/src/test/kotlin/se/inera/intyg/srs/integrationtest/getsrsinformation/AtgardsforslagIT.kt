...
class AtgardsforslagIT : BaseIntegrationTest() {
    ...
    fun testRequestAtgardsforslag() {
        ...
        .then()
            ...
            .assertThat()
                ...
                .body("$ATGARD_ROOT.rekommendation[0].atgardsrekommendationstatus", equalTo("OK"))
                .body("$ATGARD_ROOT.rekommendation[0].diagnos.displayName", equalTo("Riktigt stressad"))
                .body("$ATGARD_ROOT.rekommendation[0].atgard[0].atgardsforslag", equalTo("Ta det lugnt"))
                .body("$ATGARD_ROOT.rekommendation[0].atgard[0].prioritet", equalTo("1"))
                .body("$ATGARD_ROOT.rekommendation[0].atgard[1].atgardsforslag", equalTo("Hetsa inte upp dig"))
                .body("$ATGARD_ROOT.rekommendation[0].atgard[1].prioritet", equalTo("2"))

    }

    @Test
    fun testRequestAtgardsforslagMissing() {
        addMeasure("M75", "Riktigt stressad",
            listOf(Pair("Relaxa","Ta det lugnt"), Pair("Chilla","Hetsa inte upp dig")))

        RestAssured.given()
            .contentType(ContentType.XML)
            .body(getClasspathResourceAsString("atgardsforslag/getAtgardsforslagMissingRequest.xml")
                .replace("diagnosis_placeholder", "F43.8A"))
            .whenever()
            .post("/services/getsrs")
            .then()
            .statusCode(200)
    ...
    fun testRequestAtgardsforslagHigherDiagnoseLevel() {
        ...
        .then()
            ...
            .assertThat()
                .body("$SOAP_ROOT.resultCode", equalTo("OK"))
                .body("$ATGARD_ROOT.rekommendation[0].atgardsrekommendationstatus", equalTo("DIAGNOSKOD_PA_HOGRE_NIVA"))
                .body("$ATGARD_ROOT.rekommendation[0].diagnos.displayName", equalTo("Riktigt stressad"))
                .body("$ATGARD_ROOT.rekommendation[0].atgard[0].atgardsforslag", equalTo("Ta det lugnt"))
                .body("$ATGARD_ROOT.rekommendation[0].atgard[0].prioritet", equalTo("1"))
                .body("$ATGARD_ROOT.rekommendation[0].atgard[0].version", notNullValue())
                .body("$ATGARD_ROOT.rekommendation[0].atgard[1].atgardsforslag", equalTo("Hetsa inte upp dig"))
                .body("$ATGARD_ROOT.rekommendation[0].atgard[1].prioritet", equalTo("2"))
                .body("$ATGARD_ROOT.rekommendation[0].atgard[1].version", notNullValue())
