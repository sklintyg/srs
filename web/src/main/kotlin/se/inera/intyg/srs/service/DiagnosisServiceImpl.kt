...
class DiagnosisServiceImpl(val diagnosisRepository: DiagnosisRepository) {
     ...
     */
    fun getModelForDiagnosis(diagnosisId: String, modelVersion: String): PredictionDiagnosis? {
        val log = LoggerFactory.getLogger(javaClass)
        val MAX_ID_POSITIONS = 5
        val MIN_ID_POSITIONS = 3
        var currentId = this.cleanDiagnosisCode(diagnosisId)

        if (currentId.length == 3) { // 3 = no subdiagnosis
            log.debug("getModelForDiagnosis will fetch model for $diagnosisId without subdiagnosis")
            return diagnosisRepository.findOneByDiagnosisIdAndModelVersionAndForSubdiagnosis(currentId, modelVersion, false)
        } else { // if we have 4 or more characters in the diagnosis code we try to find a model with subdiagnosis if available
            log.debug("getModelForDiagnosis will look for model for $diagnosisId with subdiagnosis")
